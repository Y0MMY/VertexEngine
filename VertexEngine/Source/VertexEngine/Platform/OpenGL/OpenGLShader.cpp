...
namespace Vertex 
{
	...
	void OpenGLShader::UploadUniformFloat2(const std::string& name, const Vector2<float>& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
		glUniform2f(location, value.x, value.y);
	}

	void OpenGLShader::UploadUniformFloat3(const std::string& name, const Vector3<float>& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
		glUniform3f(location, value.x, value.y, value.z);
	}

	void OpenGLShader::UploadUniformFloat4(const std::string& name, const Vector4<float>& value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
	...
	void OpenGLShader::SetFloat2(const std::string& name, const Vector2<float>& value)
	{
		UploadUniformFloat2(name, value);
	}

	void OpenGLShader::SetFloat3(const std::string& name, const Vector3<float>& value)
	{
		UploadUniformFloat3(name, value);
	}

	void OpenGLShader::SetFloat4(const std::string& name, const Vector4<float>& value)
	{
	...
	void OpenGLShader::ReadShaderFromFile(const std::filesystem::path& filepath)
	{
		...
		{
			VE_CORE_WARN("Could not read shader file {0}", filepath.string());
		}
	}
	...
	void OpenGLShader::CompileAndUploadShader()
	{
		...
		while (pos != std::string::npos)
		{
			...
			VE_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel", "Invalid shader type specified");
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				VE_CORE_ERROR("Shader compilation failed:\n{0}", &infoLog[0]);

				// We don't need the shader anymore.
				glDeleteShader(shaderRendererID);

				VE_CORE_ASSERT(false, "Failed");
			}
